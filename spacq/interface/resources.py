...
class Resource(object):
        ...
        try:
            if self.units is not None:
                try:
                    ...
                    raise TypeError(
                        'Expected a Quantity, not "{0}"'.format(value))
                except IncompatibleDimensions:
                    raise TypeError(
                        'Expected dimensions matching "{0}", not "{1}"'.format(self.units, value))
            else:
    ...
    def value(self, v):
        ...
        if self.allowed_values is not None and v not in self.allowed_values:
            raise ValueError('Given disallowed value: {0}. Allowed values are: {1}'.format(
                v, self.allowed_values))
    ...
    def wrapped(self, name, getter_filter=None, setter_filter=None):
        ...
        result.wrappers = self.wrappers + \
            [(name, getter_filter, setter_filter)]
    ...
    def sweep(self, value_from, value_to, steps, delay=0.1, exception_callback=None):
        ...
        quantites_values = linspace(
            value_from.original_value, value_to.original_value, steps)
        quantites_list = [
            Quantity(value, units=value_from.original_units) for value in quantites_values]
            ...
            with self.running_lock:
                ...
                if self.resource is not None:
                    try:
                        ...
                        log.error(
                            'Could not obtain resource value: {0!r}'.format(e))
                    else:
                        self.callback(value)
