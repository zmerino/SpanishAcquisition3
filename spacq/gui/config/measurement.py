...
class MeasurementConfigPanel(wx.Panel):
    ...
    def __init__(self, parent, global_store, scaling=True, *args, **kwargs):
        ...
        if self.scaling:
            self.scaling_wrap_token = '{0}.{1}'.format(
                self.__class__.__name__, self.wrap_with_scaling.__name__)
        self.resource = None
        ...
        configuration_box.Add(self.enabled_checkbox,
                              flag=wx.CENTER | wx.RIGHT, border=15)

        self.Bind(wx.EVT_CHECKBOX, self.OnCaptureChecked,
                  self.enabled_checkbox)

        # Names.
        names_box = wx.FlexGridSizer(rows=2, cols=2, vgap=5, hgap=5)
        ...
        self.resource_name_input = wx.TextCtrl(
            self, value=self.var.resource_name, style=wx.TE_PROCESS_ENTER)
        ...
        self.Bind(wx.EVT_TEXT, self.OnResourceNameChange,
                  self.resource_name_input)
        self.Bind(wx.EVT_TEXT_ENTER, self.OnResourceNameInput,
                  self.resource_name_input)

        names_box.Add(wx.StaticText(self, label='Measurement name:'),
                      flag=wx.ALIGN_CENTER_VERTICAL | wx.ALIGN_RIGHT)
        self.measurement_name_input = wx.TextCtrl(
            self, value=self.var.name, style=wx.TE_PROCESS_ENTER)
        ...
        self.Bind(wx.EVT_TEXT, self.OnMeasurementNameChange,
                  self.measurement_name_input)
        self.Bind(wx.EVT_TEXT_ENTER, self.OnMeasurementNameInput,
                  self.measurement_name_input)

        # Scaling.
        if self.scaling:
            scaling_button = wx.Button(
                self, label='Scaling...', style=wx.BU_EXACTFIT)
            self.Bind(wx.EVT_BUTTON, self.OnScaling, scaling_button)
            configuration_box.Add(
                scaling_button, flag=wx.EXPAND | wx.LEFT, border=10)
