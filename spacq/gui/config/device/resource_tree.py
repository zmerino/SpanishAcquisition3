...
class ResourceTree(TreeListCtrl):
    ...
    def build_tree(self, device, resource_labels, root=None, path=None):
        ...
        for name, resource in list(device.resources.items()):
            ...
            if full_path in resource_labels:
                self.SetItemText(
                    item, resource_labels[full_path], self.col_label)
        ...
        def update():
            try:
                ...
                if error_callback is not None:
                    error_callback(ValueError(
                        'Expected dimensions to match "{0}"'.format(resource.units)))
                else:
                    raise
            ...
            try:
                ...
                wx.CallAfter(self.SetItemText, item,
                             true_value, self.col_value)
    ...
    def OnBeginLabelEdit(self, evt):
        ...
        if evt.Int == self.col_label:
            ...
            if resource.allowed_values is not None:
                ...
                try:
                    dlg.SetSelection(options.index(
                        self.GetItemText(evt.GetItem(), self.col_value)))
                except ValueError:
                    pass
    ...
    def OnEndLabelEdit(self, evt):
        if self.editing_col == self.col_label:
            ...
            self.set_value(evt.GetItem(), value, error_callback=partial(
                wx.CallAfter, error_callback))
