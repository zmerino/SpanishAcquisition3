...
class DevicesPanel(wx.Panel):
    ...
    def __init__(self, parent, global_store, dialog_owner, *args, **kwargs):
        ...
        self.olv.SetColumns(
            [self.col_name, self.col_connection, self.col_setup, self.col_status])
    ...
    def update_resources(self, old, new):
        ...
        with self.global_store.resources.lock:
            ...
            conflicting_resources = [
                label for label in appeared if label in self.global_store.resources]
    ...
    def OnCellEditStarting(self, evt):
        ...
        if col == self.col_connection:
            def ok_callback(dlg):
                ...
                if conflicting_resources:
                    MessageDialog(self, ', '.join(
                        conflicting_resources), 'Conflicting resources').Show()
            ...
            if dev.gui_setup is not None:
                dev.gui_setup(self.dialog_owner,
                              self.global_store, dev.name).Show()
    ...
    def OnCellEditFinishing(self, evt):
        ...
        if col == self.col_name:
            ...
            try:
                ...
                MessageDialog(self, dev_new_name,
                              'Device name conflicts').Show()
                evt.Veto()
                return
    ...
    def OnRemoveDevices(self, evt=None):
        ...
        if connected_devices:
            MessageDialog(self, ', '.join(sorted(connected_devices)),
                          'Devices still connected').Show()
            return
