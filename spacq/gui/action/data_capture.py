...
class DataCaptureDialog(Dialog, SweepController):
    ...
    def __init__(self, parent, resources, variables, num_items, measurement_resources,
        ...
        # self.general_exception_handler = partial(
        #     wx.CallAfter, self._general_exception_handler)
        self.resource_exception_handler = partial(
            wx.CallAfter, self._resource_exception_handler)
        ...
        self.progress_bar = wx.Gauge(
            self, range=num_items, style=wx.GA_HORIZONTAL)
        ...
        # Values.
        self.values_box = wx.FlexGridSizer(
            cols=2, vgap=5, hgap=20)  # rows=len(self.variables)
        ...
        # Separator.
        for _ in range(2):
            self.values_box.Add(wx.StaticLine(
                self), flag=wx.EXPAND | wx.ALL, border=5)
        ...
        # Times.
        times_box = wx.FlexGridSizer(
            rows=2 if self.show_remaining_time else 1, cols=2, vgap=5, hgap=5)
        dialog_box.Add(times_box, proportion=1,
        ...
        # Last continuous.
        if self.continuous:
            self.last_continuous_input = wx.CheckBox(
                self, label='Last loop of continuous sweep')
    ...
    def _general_exception_handler(self, f, e):
        ...
        """
        MessageDialog(self.parent, '{0}'.format(
            str(e)), 'Sweep error in "{0}"'.format(f)).Show()
    ...
    def _resource_exception_handler(self, resource_name, e, write=True):
        ...
        MessageDialog(self.parent, msg,
                      'Error {0} resource'.format(dir)).Show()
        ...
        if self.num_items > 0 and self.item >= 0:
            ...
            if self.last_checked_time > 0:
                self.elapsed_time += int((time() -
                                         self.last_checked_time) * 1e6)
                self.elapsed_time_output.Label = str(
                    timedelta(seconds=self.elapsed_time//1e6))
            ...
            if self.show_remaining_time and amount_done > 0:
                ...
                self.remaining_time_output.Label = str(
                    timedelta(seconds=remaining_time//1e6))
...
class DataCapturePanel(wx.Panel):
    ...
    def __init__(self, parent, global_store, *args, **kwargs):
        ...
        panel_box.Add(export_box, proportion=1,
        ...
        # Directory.
        self.directory_browse_button = DirBrowseButton(
            self, labelText='Directory:')
    ...
    def OnBeginCapture(self, evt=None):
        ...
        all_variables = [var for var in list(
            self.global_store.variables.values()) if var.enabled]
        output_variables = sift(all_variables, OutputVariable)
        input_variables = [var for var in sift(
            all_variables, InputVariable) if var.resource_name != '']
        condition_variables = sift(all_variables, ConditionVariable)

        if not output_variables:
            output_variables.append(OutputVariable(
                order=0, name='<Dummy>', enabled=True))
        ...
        resource_names = [tuple(var.resource_name for var in group)
                          for group in output_variables]
        measurement_resource_names = [
            var.resource_name for var in input_variables]
        condition_resource_names = [tuple(set(flatten(
            [var.resource_names for var in group]))) for group in condition_variables]
        ...
        if pulse_program is not None:
            ...
            try:
                ...
                MessageDialog(self, '\n'.join(
                    e[0]), 'Pulse program error', monospace=True).Show()
                return
            ...
            try:
                ...
                pulse_channels = dict((k, v) for k, v in list(
                    pulse_program.output_channels.items()) if v is not None)

                actual_channels = list(range(1, len(pulse_awg.channels)))
                invalid_channels = [k for k, v in list(
                    pulse_channels.items()) if v not in actual_channels]

                if invalid_channels:
                    MessageDialog(self, 'Invalid channels for: {0}'.format(
                        ', '.join(invalid_channels)), 'Invalid channels').Show()
                    return
            ...
            try:
                pulse_config = PulseConfiguration(
                    pulse_program, pulse_channels, pulse_awg, pulse_oscilloscope)
            except TypeError as e:
                MessageDialog(
                    self, str(e), 'Device configuration error').Show()
                return
        else:
            pulse_config = None
        ...
        for items, msg in [
            ...
            if items:
                MessageDialog(self, ', '.join('"{0}"'.format(
                    x) for x in sorted(items)), msg).Show()
        ...
        for cvar in flatten(condition_variables):
            ...
            for cond in cvar.conditions:
                ...
                if cond.type1 == 'resource name':
                    resource1 = [resource for (name, resource) in flatten(
                        condition_resources) if name == cond.arg1][0]
                    value1 = resource1.value
                if cond.type2 == 'resource name':
                    resource2 = [resource for (name, resource) in flatten(
                        condition_resources) if name == cond.arg2][0]
                ...
                if hasattr(resource1, 'allowed_values') and resource1.allowed_values is not None:
                    if value2 not in resource1.allowed_values:
                        MessageDialog(self, 'In the condition {0}, {1} is not in allowed_values of {2}.'.format(
                            cond, value2, cond.arg1), 'Condition error').Show()
                        return
                if hasattr(resource2, 'allowed_values') and resource2.allowed_values is not None:
                    if value1 not in resource2.allowed_values:
                        MessageDialog(self, 'In the condition {0}, {1} is not in allowed_values of {2}.'.format(
                            cond, value1, cond.arg2), 'Condition error').Show()
                        return
                ...
                if resource1 is not None and resource1.units is not None:
                    try:
                        ...
                        MessageDialog(self, 'In the condition {0}, {1} does not have a dimension.'.format(
                            cond, value2), 'Condition error').Show()
                        return
                    except IncompatibleDimensions:
                        MessageDialog(self, 'In the condition {0}, {1} and {2} do not have matching dimensions.'.format(
                            cond, value1, value2), 'Condition error').Show()
                        return
                if resource2 is not None and resource2.units is not None:
                    try:
                        ...
                        MessageDialog(self, 'In the condition {0}, {1} does not have a dimension.'.format(
                            cond, value1), 'Condition error').Show()
                        return
                    except IncompatibleDimensions:
                        MessageDialog(self, 'In the condition {0}, {1} and {2} do not have matching dimensions.'.format(
                            cond, value1, value2), 'Condition error').Show()
        ...
        if self.export_enabled.Value:
            ...
            name = '{0:04}-{1:02}-{2:02}_{3:02}-{4:02}-{5:02}.csv'.format(
                *localtime())

            if not dir:
                MessageDialog(self, 'No directory selected.',
                              'Export path').Show()
                return

            if not os.path.isdir(dir):
                MessageDialog(self, 'Invalid directory selected',
                              'Export path').Show()
                return
        ...
        def data_callback(cur_time, values, measurement_values):
            for name, value in zip(measurement_resource_names, measurement_values):
                wx.CallAfter(pub.sendMessage, 'data_capture.data',
                             name=name, value=value)

            # Extract values out of quantities, since the units have already been taken care of in the header.
            values = [x.original_value if hasattr(
                x, 'original_value') else x for x in values]
            measurement_values = [x.original_value if hasattr(
                x, 'original_value') else x for x in measurement_values]
