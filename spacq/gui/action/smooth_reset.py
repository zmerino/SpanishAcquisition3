...
class SmoothResetPanel(wx.Panel):
    ...
    def choose_variables(self):
        ...
        all_vars = sift(
            list(self.global_store.variables.values()), OutputVariable)
        vars = [
            var for var in all_vars if var.enabled and var.use_const and var.resource_name]
            ...
            try:
                ...
                for var in vars:
                    ...
                    if sweep_setting == 1:
                        ...
                        value_from, value_to = resource.value, var.with_type(
                            var.const)

                    thr = Thread(target=resource.sweep, args=(value_from, value_to, self.reset_steps_input.Value), kwargs={
                                 'exception_callback': partial(wx.CallAfter, exception_callback)})
                    thr.daemon = True
                    thrs.append(thr)
