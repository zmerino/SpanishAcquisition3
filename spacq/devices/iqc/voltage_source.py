...
class VoltageSource(AbstractDevice):
    ...
    def _setup(self):
        ...
        for num in range(16):
            if num < 6:
                ...
                port = Port(self, num, 2.5, **self.port_settings)
            else:
                port = Port(self, num, 10, **self.port_settings)
    ...
    def ask_encoded(self, msg, assertion=None):
        ...
        if assertion is not None:
            ...
            assertion_bytes = bytes.fromhex(assertion)

            assert result == assertion_bytes, (
