...
class PortTest(TestCase):
	def testCalculateVoltage(self):
		...
		port = ch6_voltage_source.Port(None, None, apply_settings=False, resolution=20)
	...
	def testResolution(self):
		...
		port = ch6_voltage_source.Port(None, None, apply_settings=False, resolution=16)
	...
	def testVoltageBounds(self):
		...
		port = ch6_voltage_source.Port(None, None, apply_settings=False)
	...
	def testFormatForDAC(self):
		...
		for m, f in data:
			eq_(ch6_voltage_source.Port.format_for_dac(m), f)
	...
	def testCustomBounds(self):
		...
		port = ch6_voltage_source.Port(None, None, apply_settings=False, min_value=-100, max_value=-30)
