...
class VoltageSourceTunerDialog(Dialog):
    ...
    def __init__(self, parent, global_store, ok_callback, port,	*args, **kwargs):
        ...
        # Self-calibration.
        calibration_static_box = wx.StaticBox(
            self, label='DAC self-calibration')
        calibration_box = wx.StaticBoxSizer(
            calibration_static_box, wx.VERTICAL)
        ...
        autotuning_sizer = wx.FlexGridSizer(rows=3, cols=2, vgap=5, hgap=5)
        ...
        # Manual tune.
        tuning_sizer = wx.FlexGridSizer(rows=2, cols=2, vgap=5, hgap=5)
...
class VoltageSourceSettingsPanel(wx.Panel):
    ...
    def __init__(self, parent, global_store, vsrc, *args, **kwargs):
        ...
        # Ports.
        ports_box = wx.FlexGridSizer(rows=8, cols=2, vgap=5, hgap=5)
        panel_box.Add(ports_box)

        for port in range(16):
            port_static_box = wx.StaticBox(
                self, label='Port {0} '.format(port))
            port_box = wx.StaticBoxSizer(port_static_box, wx.HORIZONTAL)
            ports_box.Add(port_box, flag=wx.ALL, border=5)

            if port < 6:
                spin = FloatSpin(self, value=0, min_val=-1,
                                 max_val=1, increment=1, digits=6)
            elif port < 12:
                spin = FloatSpin(self, value=0, min_val=-2.5,
                                 max_val=2.5, increment=1, digits=6)
            else:
                spin = FloatSpin(self, value=0, min_val=-10,
                                 max_val=10, increment=1, digits=6)
            ...
            tune_button = wx.Button(
                self, label='Tune...', style=wx.BU_EXACTFIT)
        ...
        tuning_data_box = wx.StaticBoxSizer(
            tuning_data_static_box, wx.HORIZONTAL)
    ...
    def OnTune(self, port_num, evt=None):
        ...
        dlg = VoltageSourceTunerDialog(
            self, self.global_store, ok_callback, port)
...
class VoltageSourceSettingsDialog(Dialog):
    ...
    def __init__(self, parent, global_store, vsrc_name, *args, **kwargs):
        ...
        Dialog.__init__(
            self, parent, title='Voltage source settings', *args, **kwargs)
