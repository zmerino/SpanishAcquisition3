...
"""
Custom 4 channel voltage source
...
class Port(AbstractSubdevice):
    ...
    def __init__(self, device, num, resolution=20, apply_settings=True, min_value=-10,
        ...
        device: The ch4VoltageSource to which this Port belongs.
    ...
    def calculate_voltage(self, voltage):
        ...
        try:
            ...
            raise ValueError(
                'Voltage must be a number. Given: {0}'.format(voltage))
        ...
        return int(round(float(-voltage_adjusted + self.max_value) / value_span * max_converted, 0))
    ...
    def write_to_dac(self, message):
        ...
        if message_length > 4:
            raise ValueError(
                'Message is longer than 4 bytes: {0}'.format(message))
        ...
        # Note[Kyle Willick]: These Hex strings come from using NI-Spy while communicating with the NI-8451 labview VIs
        self.device.ask_encoded('0000 000c 0008 0100 0000 0000',
                                '0000 001c 0018 0100 0000 0002 0200 1000 0110 c000 0100 c000 0002 0000')
        ...
        log.debug('Ask: 0000 0014 0010 0111 0260 0000 0003 {0:02x} 00 {1} \n Receive: 0000 0014 0010 0100 0000 0002 {2:02x} 00 0000 {3}'.format(
            message_length, message_formatted, message_length, expected_reply))
    ...
    def voltage(self, value):
        ...
        resulting_voltage = self.calculate_voltage(
            value) << (20 - self.resolution)

        # Write 24 bits to the top of the DIR: 0100 0000 xxxx xxxx xxxx xxxx xxxx xxxx
        self.write_to_dac('40 {0:05x} f'.format(resulting_voltage))
...
class ch4VoltageSource(AbstractDevice):
    """
    ...
    def _gui_setup(self):
        try:
            from .gui.ch4_voltage_source import ch4VoltageSourceSettingsDialog

            return ch4VoltageSourceSettingsDialog
        except ImportError as e:
            log.debug('Could not load GUI setup for device "{0}": {1}'.format(
                self.name, str(e)))
    ...
    def ask_encoded(self, msg, assertion=None):
        ...
        if assertion is not None:
            ...
            formatted_assertion = BinaryEncoder.decode(
                BinaryEncoder.encode(assertion))
...
name = 'Four channel voltage source'
implementation = ch4VoltageSource
